...
<body x-data="{ open: false, darkMode: localStorage.getItem('dark') === 'true' }"
    ...
    :class="{ 'dark': darkMode }" class="bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-gray-100">

    <
    <button @click="darkMode = !darkMode"
        class="fixed top-4 right-4 bg-blue-500 dark:bg-yellow-400 text-white dark:text-black p-2 rounded transition-transform duration-300 hover:scale-110">
    ...
    <div class="flex">
        ...
        <div :class="open ? 'translate-x-0' : '-translate-x-full'"
            class="fixed inset-y-0 left-0 w-64 bg-gray-800 dark:bg-gray-700 text-white transform transition-transform duration-300 ease-in-out p-5 lg:translate-x-0">
        ...
        {{ request()->routeIs('admin.dashboard') ? 'bg-gray-700 font-bold border-l-4 border-blue-400' : '' }}">
                ...
                <div x-data="{ open: {{ request()->routeIs('admin.manageportal.*') ? 'true' : 'false' }} }" class="relative">
                    <a href="#" @click.prevent="open = !open"
                        class="block py-2 px-4 rounded hover:bg-gray-700 
            {{ request()->routeIs('admin.manageportal.*') ? 'bg-gray-700 font-bold border-l-4 border-blue-400' : '' }}">
                ...
                {{ request()->routeIs('admin.manageportal.index') ? 'bg-gray-600 font-bold border-l-4 border-blue-400' : '' }}">
                            <i class="fa-solid fa-sliders mr-2"></i> Slide
                        </a>
                        <a href="{{ route('admin.manageportal.gallery.index') }}"
                            class="block py-2 px-4 rounded hover:bg-gray-600 
                {{ request()->routeIs('admin.manageportal.gallery.index') ? 'bg-gray-600 font-bold border-l-4 border-blue-400' : '' }}">
                            <i class="fa-solid fa-image mr-2"></i> Gallery
                        </a>
                        <a href="{{ route('admin.manageportal.kegiatan.index') }}"
                            class="block py-2 px-4 rounded hover:bg-gray-600 
                {{ request()->routeIs('admin.manageportal.kegiatan.index') ? 'bg-gray-600 font-bold border-l-4 border-blue-400' : '' }}">
                ...
                <a href="{{ route('guru.index') }}"
                    class="block py-2 px-4 rounded hover:bg-gray-700 
        {{ request()->routeIs('guru.*') ? 'bg-gray-700 font-bold border-l-4 border-blue-400' : '' }}">
                    <i class="fa-solid fa-user-check mr-2"></i> Userguru
                </a>

                <a href="{{ route('datasantri.index') }}"
                    class="block py-2 px-4 rounded hover:bg-gray-700 
        {{ request()->routeIs('datasantri.*') ? 'bg-gray-700 font-bold border-l-4 border-blue-400' : '' }}">
                    <i class="fa-solid fa-users mr-2"></i> Data Santri
                </a>

                <a href="{{ route('kelas.index') }}"
                    class="block py-2 px-4 rounded hover:bg-gray-700 
        {{ request()->routeIs('kelas.*') ? 'bg-gray-700 font-bold border-l-4 border-blue-400' : '' }}">
                    <i class="fa-solid fa-chalkboard-user mr-2"></i> Data Kelas
                </a>

                <a href="{{ route('mapel.index') }}"
                    class="block py-2 px-4 rounded hover:bg-gray-700 
        {{ request()->routeIs('mapel.*') ? 'bg-gray-700 font-bold border-l-4 border-blue-400' : '' }}">
                    <i class="fa-solid fa-book-open mr-2"></i> Mata Pelajaran
                </a>

                <a href="{{ route('kamar.index') }}"
                    class="block py-2 px-4 rounded hover:bg-gray-700 
        {{ request()->routeIs('kamar.*') ? 'bg-gray-700 font-bold border-l-4 border-blue-400' : '' }}">
                    <i class="fa-solid fa-bed mr-2"></i> Kamar
                </a>

                <a href="{{ route('pelanggaran.index') }}"
                    class="block py-2 px-4 rounded hover:bg-gray-700 
        {{ request()->routeIs('pelanggaran.*') ? 'bg-gray-700 font-bold border-l-4 border-blue-400' : '' }}">
                    <i class="fa-solid fa-triangle-exclamation mr-2"></i> Pelanggaran
                </a>

                <a href="{{ route('tabungan.index') }}"
                    class="block py-2 px-4 rounded hover:bg-gray-700 
        {{ request()->routeIs('tabungan.*') ? 'bg-gray-700 font-bold border-l-4 border-blue-400' : '' }}">
                    <i class="fa-solid fa-piggy-bank mr-2"></i> Tabungan
                </a>

                <a href="{{ route('perizinan.tampil') }}"
                    class="block py-2 px-4 rounded hover:bg-gray-700 
        {{ request()->routeIs('perizinan.*') ? 'bg-gray-700 font-bold border-l-4 border-blue-400' : '' }}">
                    <i class="fa-solid fa-door-open mr-2"></i> Perizinan
                </a>

                <a href="{{ route('useradmin.index') }}"
                    class="block py-2 px-4 rounded hover:bg-gray-700 
       {{ request()->routeIs('useradmin.index') ? 'bg-gray-700 font-bold border-l-4 border-blue-400' : '' }}">
                    <i class="fa-solid fa-user-shield mr-2"></i> Pengurus
                </a>

                <a href="{{ route('useradmin.password.form') }}"
    class="block py-2 px-4 rounded hover:bg-gray-700 
    {{ request()->routeIs('useradmin.password.form') ? 'bg-gray-700 font-bold border-l-4 border-blue-400' : '' }}">
