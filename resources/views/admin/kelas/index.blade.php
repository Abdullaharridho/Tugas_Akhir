...
@section('konten')
<div x-data="kelasModal()" class="container mx-auto p-6">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-3xl font-bold text-gray-800 dark:text-white">📘 Daftar Kelas</h2>
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 rounded-lg">
                <thead class="bg-gray-50 dark:bg-gray-700">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-sm font-medium text-gray-700 dark:text-gray-200">Nama Kelas</th>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex gap-2">
                                <button @click="openEdit({{ $item->id }}, '{{ $item->nama }}')"
                                    class="bg-yellow-400 hover:bg-yellow-500 text-black px-4 py-1.5 rounded-full transition font-medium">
                                    ✏️ Edit
                                </button>

                                <form action="{{ route('kelas.hapus', $item->id) }}" method="POST"
                                    onsubmit="return confirm('Hapus kelas ini?')">
                                    @csrf @method('DELETE')
                                    <button type="submit"
                                        class="bg-red-600 hover:bg-red-700 text-white px-4 py-1.5 rounded-full transition font-medium">
                                        🗑️ Hapus
                                    </button>
                                </form>
                            </div>
